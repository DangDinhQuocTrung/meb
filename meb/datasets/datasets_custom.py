...
class Casme3A(dataset_utils.Dataset):
    ...
    def data_frame(self) -> pd.DataFrame:
        ...
        # print(df.shape)
        # print(df[df["Expression type"] == "Micro-expression"].shape)
        df = df.rename(
            {
                "Subject": "subject",
                "Filename": "material",
        ...
        df = df[df["Expression type"] == "Micro-expression"]
        df = df.reset_index()
        return df
...
class CrossDataset(dataset_utils.Dataset):
    ...
    def data_frame(self) -> pd.DataFrame:
        ...
        df["n_frames"] = df["n_frames"].astype(int)
        # Sort action units
        aus_columns = [col for col in df.columns if ("AU" in col and col != "AU")]
        aus_sorted = sorted(aus_columns, key=lambda x: int(x[2:]))
        meta_columns = [col for col in df.columns if not ("AU" in col and col != "AU")]
        columns_sorted = meta_columns + aus_sorted
        df = df[columns_sorted]
        # Add apex from starting 0
        df.insert(5, "apexf", df["apex"] - df["onset"])
