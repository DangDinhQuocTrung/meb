...
class Unfold(nn.Module):
    def __init__(self, kernel_size=3, mode="full"):
        ...
        assert mode in ["full", "distributed", "scaled", "half_scaled", "single"]
        self.mode = mode
        UnfoldOp.mode = mode
...
class UnfoldOp(autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        elif UnfoldOp.mode == "single":
            grad_output = rearrange(grad_output, "b (c s) h w -> b c s h w", c=c, s=s)
            grad_input = grad_output[:, :, 4, :, :]
        elif UnfoldOp.mode == "distributed":
            grad_output = rearrange(grad_output, "b (c s) h w -> b c s h w", c=c, s=s)
            grad_input = torch.mean(grad_output, dim=2)
        else:
