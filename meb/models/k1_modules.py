...
from meb.models.folding import Unfold
...
class SS2Depth_K1(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.mode = mode
        assert mode in ["head", "center", "head-center", "center-center", "split", "even"]
        # print("K1")
    ...
    def forward_core(self, x: torch.Tensor, dx: torch.Tensor):
        ...
        LL = (L + CC * 2) if self.mode in ["head-center", "center-center"] else (L + CC)
        K = 1
        half_length = L // 2
        ...
        elif self.mode == "head-center":
            dx_inv = torch.flip(dx, dims=[-1])
            xs = torch.cat([dx, xs[..., :half_length], dx_inv, xs[..., half_length:]], dim=-1)
        elif self.mode == "split":
            dx_half_length = dx.shape[-1] // 2
            dx_quar_length = dx.shape[-1] // 4
            xs = torch.cat([
                dx[..., :dx_quar_length],
                xs[..., :half_length],
                dx[..., dx_quar_length:-dx_quar_length],
                xs[..., half_length:],
                dx[..., -dx_quar_length:],
            ], dim=-1)
        elif self.mode == "even":
        ...
        if self.mode == "head":
            out_y = out_y[..., CC:]
        elif self.mode in ["head-center"]:
            out_y = torch.cat([
                out_y[..., CC:(CC + half_length)],
                out_y[..., -half_length:],
            ], dim=-1)
        elif self.mode in ["split"]:
            out_y = torch.cat([
                out_y[..., dx_quar_length:(dx_quar_length + half_length)],
                out_y[..., -(dx_quar_length + half_length):-dx_quar_length],
            ], dim=-1)
        elif self.mode in ["center", "center-center"]:
            out_y = torch.cat([
                out_y[..., :half_length],
                out_y[..., -half_length:],
            ], dim=-1)
...
class SS2DConv_K1(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        assert out_y.dtype == torch.float
        assert not torch.isnan(out_y.mean())
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        y = torch.transpose(y, dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        y = self.fold(y)
        # print("fold:", y.mean(), y.shape, torch.max(torch.nan_to_num(y)))
        assert not torch.isnan(y.mean())
        y = self.out_norm(y)
        y = y * F.silu(z)
